...
## 2026-08-07 - Optimize filter and includes overhead with Sets
**Learning:** Using `.includes()` on an array inside a `.filter()` callback creates an O(N * M) time complexity overhead when traversing large sets of state/arrays (e.g., deduplicating shopping list items against dismissed item IDs).
**Action:** Convert the lookup array into a `Set` before the loop, and replace `.filter()` with a standard `for` loop pushing to a `const filtered = []` array. This reduces the time complexity to O(N + M) and prevents intermediate array allocations generated by native chained array operations.
