...
## 2026-06-21 - Use of print and download export in React pages
**Learning:** For quickly enabling users to export CSV data or print the page directly from the client without backend processing, we can create hidden anchor tags for CSV blobs, or trigger `window.print()` from a React event handler. When exporting data from an API call, map the query results directly to an array structure to construct the CSV text before sending to blob URL.
**Action:** Can continue using Blob and window.print() patterns for simple client-side reporting and exporting features to speed up development time and offload work from the server. Use `print:hidden` tailwind classes to keep the printed page clean.
