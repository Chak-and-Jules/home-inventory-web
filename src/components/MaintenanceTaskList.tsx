...
export function MaintenanceTaskList({ inventoryItemId, showItemName }: MaintenanceTaskListProps) {
  ...
  const { data: tasks, isPending } = useQuery({
    queryKey: ['maintenance-tasks', currentHomeId, inventoryItemId],
    queryFn: async () => {
    ...
    mutationFn: (task: MaintenanceTask) => {
      if (!task.IsCompleted) {
        ...
        return api.put(`/maintenance-tasks/${task.ID}`, {
          ...
          custom_frequency_metric:
            task.Frequency === 'custom' ? task.custom_frequency_metric : null,
        });
      }
    },
    onSuccess: () => {
  ...
  if (isPending) {
    return <div className="p-4 text-center text-gray-500">{t('ui.loadingTasks')}</div>;
  }
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {showItemName && (
                <TableHead className="hidden sm:table-cell">{t('maintenance.item')}</TableHead>
              )}
              <TableHead>{t('ui.taskDescription')}</TableHead>
              <TableHead>{t('maintenance.scheduledDate')}</TableHead>
              <TableHead className="hidden sm:table-cell">{t('maintenance.frequency')}</TableHead>
              ...
              tasks.map((task) => (
                <Fragment key={task.ID}>
                  <TableRow className={cn(task.IsCompleted && 'opacity-60')}>
                    <TableCell>
                      <button
                        ...
                        aria-label={
                          task.IsCompleted ? t('ui.markAsIncomplete') : t('ui.markAsCompleted')
                        }
                      >
                      ...
                      {task.Frequency === 'custom'
                        ? t('maintenance.frequencyCustomDisplay', {
                            ...
                            metric: task.custom_frequency_metric
                              ? t(
                                  `maintenance.metric${task.custom_frequency_metric.charAt(0).toUpperCase() + task.custom_frequency_metric.slice(1)}`,
                                )
                              : '',
                          })
                        : task.Frequency
                          ? t(
                              `maintenance.frequency${task.Frequency.charAt(0).toUpperCase() + task.Frequency.slice(1)}`,
                            )
                          : '—'}
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          className="h-8 w-8"
                          aria-label={t('maintenance.editTask')}
                        >
                        ...
                        <Button
                          ...
                          aria-label={t('ui.deleteMaintenanceTask')}
                        >
                  ...
                  {editingTask?.ID === task.ID && (
                    <TableRow>
                      <TableCell colSpan={showItemName ? 6 : 5}>
                        <MaintenanceTaskForm
                          key={task.ID}
                          inline
                          isOpen
