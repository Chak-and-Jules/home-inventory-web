...
export function MaintenanceTaskForm({
  ...
  const handleAddDependency = () => {
    ...
    if (dependencies.some((dep) => dep.item_definition_id === selectedItemDefId)) {
      alert(t('ui.thisItemIsAlreadyAddedAsADependency'));
      return;
    }
    setDependencies([
      ...dependencies,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (frequency === 'custom') {
      ...
      if (isNaN(numVal) || numVal <= 0) {
        alert(t('ui.customFrequencyPositive'));
        return;
      }
      if (!customFrequencyMetric) {
        alert(t('ui.customFrequencyRequired'));
        return;
      }
    }
    ...
    >
      ...
      <div className="space-y-4 border-t border-gray-200 dark:border-gray-800 pt-4">
        <h4 className="font-semibold text-sm text-gray-900 dark:text-gray-100">
          {t('maintenance.dependenciesTitle')}
        </h4>
        ...
        {dependencies.length > 0 ? (
          <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
            {dependencies.map((dep) => {
              ...
              return (
                <div
                  key={dep.item_definition_id}
                  className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded-md border border-gray-100 dark:border-gray-700 text-sm"
                >
                  <span className="font-medium text-gray-900 dark:text-gray-100">
                    {itemDef ? itemDef.Name : t('ui.loading')}
                  </span>
                  <div className="flex items-center gap-3">
                    <span className="text-gray-500 dark:text-gray-400">
                      {t('ui.qty')}
                      {dep.quantity_required} {itemDef?.SizeUnit?.Name || ''}
                    </span>
                    <Button
                      type="button"
                      variant="ghost"
                      ...
                      aria-label={t('ui.removeDependency', { name: itemDef?.Name || '' })}
                    >
                      &times;
                    </Button>
                  </div>
        ...
        <div className="flex flex-col gap-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-800">
          <div className="space-y-1">
            <Label htmlFor="dependency-item" className="text-xs">
            ...
            <Select
              ...
              aria-label={t('ui.selectItemDependency')}
            >
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 space-y-1">
              <Label htmlFor="dependency-quantity" className="text-xs">
                {t('maintenance.quantityRequired')}
              </Label>
              <Input
                ...
                aria-label={t('ui.quantityRequired')}
              />
            </div>
            <Button
  ...
  return (
    ...
    >
      <DialogContent aria-describedby={undefined} className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t(task ? 'maintenance.editTask' : 'maintenance.addTask')}</DialogTitle>
        </DialogHeader>
        {form}
      </DialogContent>
    </Dialog>
