...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Check if we have a pending Google link
      if (localStorage.getItem('pending_google_link') === 'true') {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50/50 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute -top-24 -left-24 w-96 h-96 rounded-full bg-indigo-100 blur-3xl opacity-50 pointer-events-none" />
      <div className="absolute -bottom-24 -right-24 w-96 h-96 rounded-full bg-blue-100 blur-3xl opacity-50 pointer-events-none" />
      ...
      <div className="max-w-md w-full space-y-8 bg-white/80 backdrop-blur-xl p-10 rounded-3xl shadow-xl border border-gray-100 relative z-10">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="email-address">{t('auth.emailAddress')}</Label>
              <Input
                id="email-address"
                name="email"
                type="email"
        ...
        <Button
          type="button"
          variant="outline"
          className="w-full h-11 text-base font-semibold border-gray-200 hover:bg-gray-50 flex items-center justify-center gap-2 transition-transform hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
